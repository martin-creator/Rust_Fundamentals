...
pub async fn call_gpt(messages: Vec<Message>) -> Result<String, Box<dyn std::error::Error + Send>> {
    ...
    let api_key: String = env
        ::var("OPEN_AI_KEY")
        .expect("OPEN_AI_KEY not found in enviornment variables");
    let api_org: String = env
        ::var("OPEN_AI_ORG")
        .expect("OPEN_AI_ORG not found in enviornment variables");
    ...
    headers.insert(
        ...
        HeaderValue::from_str(&format!("Bearer {}", api_key)).map_err(
            |e| -> Box<dyn std::error::Error + Send> { Box::new(e) }
        )?
    );
